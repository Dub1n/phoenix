...
3. Define or refine Stage 4 lanes with `npm run consolidate -- create-lane <patternId> 4a --scope "Prerequisite scope" --command "Command to execute" [--depends 12:stage-4]` before handing off. Repeat for each required lane; the CLI refuses empty command lists.
4. When finalizing Stage 3, ensure each Stage 4 lane has concrete tasks, command list, and dependencies. CLI blocks Stage 3 completion if any Stage 4 lane lacks `scope` or `commands`.
5. On completion, mark Stage 3 `complete`. CLI records timestamp and prompts for coordination notes (referencing dependency pattern IDs).
...
Add parallel prerequisites with `npm run consolidate -- create-lane <patternId> 4d --scope "Additional prerequisite" --command "Command to validate"` as new work emerges; stage gates stay aligned when every lane carries scope, commands, and optional dependencies.
...
3. Stand up Stage 6 migration lanes with `npm run consolidate -- create-lane <patternId> 6a --scope "Migration slice" --command "Command to certify"` (repeat for `6b`–`6d` as needed). Keep each lane `pending` while it gathers gating commands and dependencies.
4. Stage 5B: verify Stage 6 lanes `status` remain `pending` but have `commands` flagged as gating tests. Execute gating battery and record logs through CLI.
5. Mark Stage 5 `ready` once all acknowledgements and evidence exist. CLI prevents `complete` until Stage 6 lanes begin.
...
1. Review the slate with `npm run consolidate -- guide <patternId> --lanes`; if a fresh migration slice is required, add it first via `npm run consolidate -- create-lane <patternId> 6d --scope "Parallel track" --command "Command to validate"` so dependencies stay enforced.
2. Claim the lane with `npm run consolidate -- claim-lane <patternId> [laneId] --agent <name>` so the status flips to `in_progress` safely (auto-skipping lanes whose dependencies are still blocked).
3. Execute the required commands; provide log paths and exit codes when the CLI prompts.
4. Finish with `npm run consolidate -- update-lane <patternId> <laneId> --status complete --summary "..." --files tmp/...log`. The CLI enforces dependencies (e.g., Lane 6c requires Patterns 6/7 Stage 5 `ready`); overrides require `--force` and add an audit note.
5. If new prerequisites emerge, set the lane `blocked`; the CLI prompts for Stage 3 updates before allowing further Stage 6 transitions. When blockers clear, dependent lanes auto-transition to `scheduled` and become eligible for `claim-lane`.
