...
- The CLI automatically reopens and blocks all downstream gates (Stage 5 onward), resets every linked cohort Stage 5A segment, and mirrors the Stage 5/6 reopen across cohort peers whenever Stage 3 leaves `complete`/`ready`. That keeps every pattern in the cohort paused until the refreshed plan lands, while Stage 4/6 lanes stay closed until Stage 3 recreates them.
...
- If Stage 5 slips back to `pending`/`blocked`, expect Stage 6 and Stage 7 gates plus the owning cohort’s Stage 5A segment (and all cohort peers’ Stage 5/6 gates) to reopen automatically; keep the hand-off doc and `update-handoff` metadata in sync while the upstream work is restored.
...
- Reopening a stage now cascades automatically: any downstream gate that was previously `complete`/`ready` is reset to `pending` (and auto-blocked) so the schedule reflects the open prerequisite. Linked cohort Stage 5A segments are also reset to `pending`, and any cohort peers with an auto-reopened Stage 5 will have their Stage 6/7 gates reset as well. Stage 4 and Stage 6 lanes are **not** reopened—Stage 3 recreation still generates the replacement lanes when needed.
