...
} from './modules/plan-file-utils.mjs';
import {
...
import {
  ...
  touchCohort,
  touchPattern
} from './modules/registry-runtime.mjs';


async function saveRegistry(registry, options = {}) {
  autoUpdateAllPatternStatuses(registry);
  const defaultRegenArgs = await persistRegistry(registry);
  if (!options.skipRegen) {
    const baseRegenArgs =
      options.regenArgs !== undefined ? options.regenArgs : defaultRegenArgs;
    const regenRequest =
...
const stageActionGuidance = {
  ...
  '2': (pattern) => [
    ...
    `3. Execute the suites via \`node scripts/run-with-timeout.mjs --preset <preset> --log-file archive/dev-files/utility-migration/evidence/pattern-${pattern.patternId}/stage2/<name>.log -- npm …\` (e.g., \`--preset jest-ci\` for \`npm run test:ci\`) and pass that same canonical archive path to \`npm run consolidate -- update-stage ${pattern.patternId} 2 --status in_progress --files <log>\` as you collect artefacts.`,
  ...
  '4': (pattern, context = {}) => {
    ...
    return [
      ...
      `3. Implement or refresh the guardrail and run it via \`node scripts/run-with-timeout.mjs --preset <preset> --log-file archive/dev-files/utility-migration/evidence/pattern-${pattern.patternId}/stage4/lane${laneId}/<name>.log -- …\` (pick \`jest-suite\`, \`jest-ci\`, or \`phase6-validation\` as needed); pass that same canonical archive path to \`npm run consolidate -- update-lane ${pattern.patternId} ${laneId} --status in_progress --files <log> --summary "Expected failure: <signature>"\`.`,
  ...
  '5': (pattern) => [
    ...
    `4. Replay the guardrail battery via \`node scripts/run-with-timeout.mjs --preset <preset> --log-file archive/dev-files/utility-migration/evidence/pattern-${pattern.patternId}/stage5/<name>.log -- …\` (e.g., \`--preset phase6-validation -- npm run phase6-validation\`) to confirm the unmigrated failure still reproduces; pass that same canonical archive path to \`npm run consolidate -- update-stage ${pattern.patternId} 5 --status in_progress --files <log>\`.`,
  ...
  '6': (pattern, context = {}) => {
    ...
    return [
      ...
      `2. Reproduce the Stage 4 guardrail failure via \`node scripts/run-with-timeout.mjs --preset <preset> --log-file archive/dev-files/utility-migration/evidence/pattern-${pattern.patternId}/stage6/lane${laneId}/<name>.log -- …\` and pass that same canonical archive path to \`npm run consolidate -- update-lane ${pattern.patternId} ${laneId} --status in_progress --files <fail-log> --summary "Baseline failure: <signature>"\`.`,
  ...
  '7': (pattern) => [
    ...
    `2. Execute the Stage 7 validation battery through \`node scripts/run-with-timeout.mjs --preset <preset> --log-file archive/dev-files/utility-migration/evidence/pattern-${pattern.patternId}/stage7/<name>.log -- …\` (match the preset to each command) and pass that same canonical archive path to \`npm run consolidate -- update-stage ${pattern.patternId} 7 --status in_progress --files <log>\`.`,
