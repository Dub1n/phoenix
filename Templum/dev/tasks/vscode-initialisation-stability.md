...
- [ ] Extend the VSCode adapter abstraction in `src/interfaces/vscode-adapter-abstracted.ts` (`VSCodeInterfaceAdapter.loadInitialContent`, `VSCodeInterfaceAdapter.applySkin`) to queue calls until the WebView signals ready, preventing early postMessage rejections that currently manifest as "WebView load" errors.
...
- Implementation: `activate` still bails early when no workspace is open, preventing WebView registration; WebView providers emit "adapter/orchestrator not ready" warnings under test, and message-queue guards are missing.
- Tests: No targeted VSCode initialisation suites exist. Running `scripts/run-with-timeout.mjs --timeout 180000 -- npm test -- tests/interfaces/interface-adapter-integration.test.ts` surfaces the WebView warning spam and leaves the process hanging.
- Tasks ahead: add focused activation tests with mocked VSCode APIs, harden WebView readiness guards, and ensure teardown clears listeners so Jest exits without manual timeouts.
