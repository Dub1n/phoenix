...
- 2025-10-15: Seeded Stage 6 baseline artefact at `validation-reports/phase6-baselines/phase5-stage6-baseline.json` and re-wired the Phase 6 CLI + simple runner to emit `.raw.json` payloads alongside summaries (baseline metadata + definitions now included). Raw instrumentation remains mock-only until partner services open the real-backend window.
