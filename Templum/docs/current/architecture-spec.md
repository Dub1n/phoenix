...
- Skin payload consumption now flows through `TemplumCore` → `UniversalSkinEngine`; adapters render cached backend skins without bespoke fallbacks while live partner endpoint validation remains pending. Templum owns the canonical `UniversalSkinDefinition` JSON contract (`universal-skin-definition.schema.json`); partner systems conform to that schema rather than publishing their local TypeScript payload interfaces as canonical. Haruspex now emits a schema-conforming payload from `provideSkinDefinition()`, with live `/getSkinDefinition` verification still tracked under the Haruspex handshake work. A dedicated `cli-generator` module (`src/interfaces/cli-generator.ts`) now converts complete `UniversalSkinDefinition` payloads into CLI menu graphs, command bindings, and shortcut definitions, and `TemplumCore.loadSkin`/`TemplumAdapterRegistry.buildInterfaceAdapters` inject this generator so `CLIInterfaceAdapter.applySkin` hydrates the menu registry and keyboard map without hardcoded defaults. The universal layout engine normalises menu metadata before routing, keeping CLI/VSCode rendering consistent and compatibility mode reporting aligned with incoming schemas. Procedural CLI windows are generated via `ContentLayoutSystem.composeWindow` and `UniversalLayoutEngine.renderForCLI`, with `EnhancedWindowSystem.renderWindowSet` exposing capability-aware window sets to the CLI adapter’s `renderMenuWindow` path; legacy fallback banners have been removed in favour of a skin-driven warning. Guardrail suites (`tests/rendering/skin-payload-consumption.integration.test.ts`, `tests/cli/cli-generator.integration.test.ts`, `tests/e2e/e2e-complete-workflows.test.ts`) exercise the abstraction path directly, emitting deterministic CLI markers and capturing metadata-driven output. **Status:** Present (awaiting live backend endpoint validation; e2e validated).
