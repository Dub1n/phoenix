...
- Skin payload consumption now flows through `TemplumCore` → `UniversalSkinEngine`; adapters render cached backend skins without bespoke fallbacks while partner exports remain pending. The universal layout engine now converts full `UniversalSkinDefinition` structures into canonical menu definitions (normalising `layout.items`, coercing menu themes, and preserving metadata) before layout/routing, so CLI/VSCode rendering stays consistent and compatibility mode reporting reflects the incoming schema. Procedural CLI windows are generated via `ContentLayoutSystem.composeWindow` and `UniversalLayoutEngine.renderForCLI`, with `EnhancedWindowSystem.renderWindowSet` exposing capability-aware window sets to the CLI adapter’s `renderMenuWindow` path; legacy fallback banners have been removed in favour of a skin-driven warning. Guardrail suites (`tests/rendering/skin-payload-consumption.integration.test.ts`, `tests/e2e/e2e-complete-workflows.test.ts`) exercise the abstraction path directly, emitting deterministic `CLI_RENDER` markers and capturing procedural window output. **Status:** Present (awaiting live backend payloads; e2e validated).
