...
| `node -e "const fs=require('fs'); for (const f of fs.readdirSync('schemas').filter(f=>f.endsWith('.json'))) JSON.parse(fs.readFileSync('schemas/'+f,'utf8'))"` | Parse public JSON contract artifacts                                            | `schemas` is reserved for public JSON contracts owned by Templum; source schema helpers remain runtime validation code                                                               |
...
| `npm run phase6-health`                                                                                                                                   | Probe service readiness via the compiled Phase 6 harness                        | Rebuilds first; exits 0 when every mock/real service reports healthy (uses `report.status=passed`). Supply `-- --baseline <file>` to override the default Stage 6 baselines when needed. |
| `npm run phase6-validation`                                                                                                                               | “Phase 6” orchestrated smoke validation (build + scripted integration harness)  | Rebuilds first; emits summary + `<timestamp>.raw.json` metrics. Defaults to mocked backends + Stage 6 baselines at `validation-reports/phase6-baselines/phase5-stage6-baseline.json`; pass `-- --use-real-backends` (and optionally `--baseline`) for live runs.        |
| `npm run phase6-validation:full`                                                                                                                          | Full Phase 6 workflow with real backend processes                               | Starts minimal backend services automatically; significantly longer. Writes paired summary/raw artefacts just like `phase6-validation`.                                               |
...
- For the backend discovery/router bundle, start with  
  ``node scripts/run-with-timeout.mjs --timeout 30000 --log-file archive/dev-files/utility-migration/evidence/pattern-2/stage7/backend-suite.log -- npm test -- --runTestsByPath src/tests/backend/service-discovery.test.ts src/tests/backend/generic-backend-integration.test.ts src/tests/backend/backend-dependency-integration.test.ts --runInBand --detectOpenHandles --forceExit --no-cache``.
...
4. Inspect the generated reports:
   - `validation-reports/phase6-validation-*.{json,md,html}` contain the typed output (look for `report.status` instead of the deprecated readiness score). Each run also emits a `phase6-validation-*.raw.json` file with full workflow/performance/cross-interface metrics plus the baseline metadata used for comparisons.
   - Baseline defaults live at `validation-reports/phase6-baselines/phase5-stage6-baseline.json`. Override with `-- --baseline <path>` during experiments; keep replacements committed so the monitor diff stays deterministic.
...
- **Watchdog long runners:** Wrap suspicious commands with `node scripts/run-with-timeout.mjs --timeout 30000 --kill-after 5000 --heartbeat 5000 --log-file archive/dev-files/utility-migration/evidence/pattern-2/stage7/backend-suite.log -- <command…>` to capture heartbeats and ensure signals propagate to hung children. The wrapper accepts any layout below a directory segment named `archive` within this monorepo, allowing each project and workflow to own an appropriate archival structure while rejecting temporary, source-tree, similarly named, and external destinations. Utility-migration runs should continue using their canonical `pattern-<id>/stage<id>/[lane<id>/]<name>.log` layout. When running the contract bundle, prefer the `--runInBand --forceExit` invocation above so Jest exits cleanly once the suites pass.
...
- **Watchdog workflow:** From the repository root, use `node Templum/scripts/run-with-timeout.mjs --timeout 30000 --kill-after 5000 --heartbeat 5000 --log-file Templum/archive/dev-files/utility-migration/evidence/pattern-2/stage7/backend-suite.log --cwd Templum -- npx jest --runInBand --runTestsByPath src/tests/backend/comprehensive-backend-validation.test.ts --testNamePattern "Pattern 11 Phase 2"` to gather heartbeat logs (and confirm signals are honoured) whenever the backend suite behaves unexpectedly.
