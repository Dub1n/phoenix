...
| `npm run test:ci [-- <pattern>]`                                                                                                                          | CI-safe Jest run with leak guard                                                | Delegates to `scripts/run-jest-ci.mjs`; sets `CI=1`, `JEST_FORCE_EXIT=1`, adds `--runInBand --detectOpenHandles --forceExit`, fails if `tests/globalTeardown.ts` finds active handles |
