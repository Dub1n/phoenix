...
export class E2ETestFramework extends EventEmitter {
  ...
  async teardownE2EEnvironment(): Promise<void> {
    ...
    for (const [name, service] of Array.from(this.mockBackends.entries())) {
      await service.stop();
    }
  ...
  private async executeInitializeAction(action: E2ETestAction): Promise<void> {
  ...
  private async executePerformanceCheckAction(action: E2ETestAction): Promise<void> {
