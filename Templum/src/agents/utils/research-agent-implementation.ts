...
// Main ResearchAgent execution with input validation, error handling, and structured output generation
// Implements generic-agent-template-pattern with structured execution pipeline

import { HandoffInput, HandoffOutput, HandoffError, HandoffErrorType } from '../../../../../../.claude/agents/interfaces/handoff-types';
import { executeResearch, ResearchResults } from './research-capabilities';
...
export async function runResearchAgent(
...
): Promise<'success' | 'partial' | 'failed' | 'retry'> {
  ...
  try {
    ...
    results = await executeResearch(input);
 ...
 */
function validateRequiredFields(input: any): void {
 ...
 */
function determineConfidence(results: ResearchResults): 'high' | 'medium' | 'low' {
  let confidenceScore = 0;
  
  // Pattern match quality
  if (results.patterns_found.length > 0) {
    const topMatch = results.patterns_found[0];
    if (topMatch.relevance_score >= 80) confidenceScore += 40;
    else if (topMatch.relevance_score >= 60) confidenceScore += 30;
    else if (topMatch.relevance_score >= 40) confidenceScore += 20;
  ...
  const totalDeps = 
    results.dependencies.file_dependencies.length +
  ...
  if (totalDeps === 0) {
    ...
    const availableDeps = 
      results.dependencies.file_dependencies.filter(d => d.status === 'available').length +
...
): 'continue' | 'fallback' | 'manual_intervention' {
  // Check for critical blockers
  const criticalBlockers = results.implementation_guidance.potential_blockers.filter(blocker =>
    blocker.includes('Missing') || blocker.includes('unavailable')
  );
 ...
 */
function generateSummary(results: ResearchResults): string {
 ...
 */
function extractRecommendations(results: ResearchResults): string[] {
 ...
 */
function getFilesAccessed(results: ResearchResults): string[] {
 ...
 */
function estimateTokenUsage(results: ResearchResults): number {
 ...
 */
async function writeOutput(filepath: string, output: HandoffOutput): Promise<void> {
  try {
...
async function handleError(
  ...
  outputFilepath: string,
  startTime: number
): Promise<'failed' | 'partial' | 'retry'> {
  const executionTime = Date.now() - startTime;
  
  let status: 'failed' | 'partial' | 'retry' = 'failed';
  let errorOutput: HandoffOutput;
 ...
 */
function getResolutionSuggestion(errorType: string): string {
  switch (errorType) {
