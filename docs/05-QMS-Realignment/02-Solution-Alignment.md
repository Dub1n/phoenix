...
This document connects each aim/objective to the capabilities of the active projects (Templum, Phoenix Code Lite, Haruspex, Validation System). The goal is to demonstrate how the existing roadmap satisfies the QMS support needs for VDL2, while exposing integration gaps inside the internal tooling stack.
...
- Highlight any places where existing scope should be trimmed or re-sequenced to serve the QMS priorities for VDL2.
