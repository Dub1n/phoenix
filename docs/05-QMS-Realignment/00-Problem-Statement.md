...
Use this worksheet to articulate why the QMS realignment effort exists and what must change. Keep the narrative tight—stakeholders should understand how the internal tooling will help the VDL2 product meet its regulatory obligations without reading other files first.

## 1. Business & Product Context

- Summarise VDL2 (medical device software), target users, deployment model, and why the upgrade is needed.
- Start from the facts in `docs/00-QMS/Info.md` (international hospitals, Rust rewrite, web + offline modes).
...
- Tie each pain point to how it slows delivery or undermines the team’s ability to demonstrate compliance for VDL2.
...
- Cite the non-negotiable standards VDL2 must satisfy: EN 62304-2006+A1:2015 as baseline, Agile integration via AAMI TIR45, ISO 13485 if applicable.
...
- Note any other systems or manual processes that must integrate with the tooling to support VDL2 compliance.

## 5. Core Problem Statement

- Express the single problem the internal QMS tooling must solve in one to three sentences.
- Format suggestion: "We must [goal] for VDL2 teams so that [regulatory/business outcome], despite [constraints]."
...
- List the measurable outcomes that prove the internal tooling is delivering value (e.g. automated traceability from requirement to validation, audit-ready evidence generated from CI).
