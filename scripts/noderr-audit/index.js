...
/*
  ...
  Usage:
    node scripts/noderr-audit/index.js [--apply] [--scan] [--verbose]
...
function countPlaceholdersInEnvironmentContext(text) {
  // Conservative heuristic: count obvious bracketed placeholders like [NodeID], [X], [Generated Timestamp]
  // Avoid counting markdown link brackets or code block examples by focusing on uppercase/wordish tokens
...
function extractEnvUrls(text) {
  // Expect YAML-like section in environment_context.md
  // local_dev_preview: url: "..." and public_deployed_app: url: "..."
...
function extractNodeIdsFromArchitecture(text) {
  // Capture Mermaid node declarations like: NODE_ID[Label] or NODE_ID(Label)
  // We avoid capturing legend identifiers starting with L_
  const regex = /\b([A-Za-z0-9_]+)\s*\[/g; // NODE_ID[ ...
  const ids = new Set();
  let m;
  while ((m = regex.exec(text)) !== null) {
    const id = m[1];
    if (id.startsWith('L_')) continue; // skip Legend helper nodes
    // Skip common non-node tokens
...
function verifyArchitecture() {
  ...
  if (!archText) {
    ...
    return { ok: false, nodeIds: [], legendPresent: false };
  }
  ...
  // TYPE_Name convention: uppercase prefix then underscore then Name (camel or words)
  let invalidCount = 0;
  ...
  // UI shape definition check (optional)
  ...
  writeText(ARCH_VERIFY_LOG, lines.join('\n'));
  return { ok: legendPresent && invalidCount === 0, nodeIds, legendPresent };
}
...
function verifyMvpAnalysis() {
  ...
  return { ok: hasMvpSection && hasMvpImplementationStatus, plannedMissing };
}

/** Optional gap scan (conservative) */
function scanForPotentialMissingComponents() {
  if (!args.scan) return [];
  // Conservative heuristic: look for top-level module directories under phoenix-code-lite/src
  // and map to known architecture TYPE prefixes. This is intentionally minimal to avoid noise.
  const candidates = [];
  ...
  const walk = (dir) => {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) walk(full);
        ...
        // Derive a simple NodeID-style suggestion from relative path
...
function buildFinalReport(context) {
  ...
  // Scoring per spec (max 100): approximate based on checks
  // Core System (30)
  ...
  // Velocity (10)
  const nextStepsScore = 5; // Always provide next steps in report
  const blockersScore = 5; // No runtime blocking in script
...
(function main() {
  ...
  mvp.hasMvpSection = /Key Features \(In Scope for MVP\)/.test(readText(PROJECT_FILE) || '');
  ...
  buildFinalReport({ env, arch: { ...arch, invalidCount: 0 }, coverage, createdSpecs, mvp });
