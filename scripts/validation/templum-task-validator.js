    ...
    try {
      ...
      if (error.message.includes('timeout')) {
        ...
        return {
          ...
          errors: [`Build timeout after ${this.options.timeout/1000}s: ${error.message}`]
        };
      }
      ...
      return {
        ...
        errors: [
          `Build failed with exit code ${error.status || 'unknown'}`,
          `Error: ${error.message}`,
    ...
    try {
      ...
      if (error.message.includes('timeout')) {
        ...
        return {
          ...
          errors: [`TypeScript timeout after ${Math.min(this.options.timeout, 45000)/1000}s: ${error.message}`]
        };
      }
