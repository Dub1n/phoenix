...
function Invoke-SafeExecution {
    ...
    $safeWrapper = "powershell -ExecutionPolicy Bypass -File `"$projectRoot\scripts\terminal-completer\core\enhanced-terminal-manager.ps1`""
    ...
    Write-Host "Executing safely with Phase 2 enhanced monitoring: $fullCommand" -ForegroundColor Cyan
...
if ($Command) {
    ...
    Write-Host "Global Command Wrapper (Phase 2 Enhanced)" -ForegroundColor Green
    ...
    Write-Host "  - Enhanced pattern recognition with context-aware matching" -ForegroundColor White
