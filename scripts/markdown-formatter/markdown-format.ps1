...
function Write-DebugMsg {
    ...
    if ($Diag -or $env:MARKDOWN_FORMAT_DEBUG -eq '1') {
        Write-Host "[MarkdownFormat][DBG] $Message" -ForegroundColor Cyan
    }
}

function Write-Info {
    param([string]$Message)
    Write-Host "[MarkdownFormat] $Message" -ForegroundColor DarkGray
}
...
$markdownTargets = $targets | Where-Object { $_.ToLower().EndsWith('.md') }
if (-not $markdownTargets -or $markdownTargets.Count -eq 0) {
    Write-Host "No markdown files found in targets. Only .md files are supported." -ForegroundColor Yellow
    exit 0
}
...
if ($Diag -or $env:MARKDOWN_FORMAT_DEBUG -eq '1') { $python = Get-PythonForDiag $python }
Write-DebugMsg "Python launcher → $python"

# Updated path since script is now in the same folder as main.py
$pyScript = Join-Path $PSScriptRoot 'main.py'
if (-not (Test-Path -LiteralPath $pyScript)) { Write-Error "main.py not found at $pyScript"; exit 1 }
...
# Add target files
$argList += $markdownTargets

Write-DebugMsg ("Args → " + ($argList -join ' '))
Write-Info ("Markdown target(s):`n - " + ($markdownTargets -join "`n - "))

# Record pre-hash to verify modifications
$preHashes = @{}
foreach ($t in $markdownTargets) { $preHashes[$t] = Get-FileHashSafe $t }
