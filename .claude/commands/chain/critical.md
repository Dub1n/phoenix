# /chain:critical - Chain-Engineered Task Orchestration with Critique
...
### Step 6: Chain Critique

**Critical Analysis Phase**: Execute harsh, objective critique of chain execution to identify improvement opportunities and verify claims.
...
Task({
  ...
  prompt: `**PERFORM HARSH CRITICAL ANALYSIS OF CHAIN EXECUTION**
          ...
          **Output Files Required**:
          - ${folderPath}/chain-results-corrected.json (critical corrections)
          - ${folderPath}/feedback-critical.md (harsh assessment)
          - ${folderPath}/improvement-action-plan.md (specific improvements)
          
          **Response Format**: Provide critical assessment summary with major problems found, missed opportunities, and improvement potential percentage.`
});
```
...
critique_integration:
  corrected_metrics:
    action: "Use Chain Critic's corrected metrics for final reporting"
    files: "chain-results-corrected.json overwrites optimistic claims"
    
  improvement_tracking:
    action: "Log identified problems for system learning"
    files: "improvement-action-plan.md provides specific fixes"
    
  quality_verification:
    action: "Independent verification of all deliverables and claims"
    standard: "Professional quality with complete requirement fulfillment"
```

### Step 7: Presentation

Give a summary of the chain results to the user and provide a link to the chain-results data and feedback form you completed.
