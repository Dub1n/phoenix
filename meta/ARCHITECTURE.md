...
- **Reality check points:**
  - Skin definitions are not yet generated by PCL/Haruspex, so Templum currently lacks live UI feeds.
...
- **Ideal Requirements (target architecture):**
  - **Universal Interface Core**
    ...
    - Enforces versioned skin contract (schema validation, backward-compatibility negotiation, error reporting).
...
- **Integration Touchpoints to Map:**
  - Auto-registration contract expected from Haruspex/PCL (HTTP endpoints, skin schema) and how Templum consumes them.
...
- **Integration Touchpoints to Map:**
  - Auto-registration and HTTP endpoints expected by Templum (health, skin, command execution).
...
- **Evidence & Documentation To Collect:**
  - Live test of backend-main service: endpoints available, sample responses.
...
- [ ] Review universal skin engine assumptions vs. actual skins to be produced; note mismatches.
...
- [ ] Outline remaining work to emit Templum-compliant skin definitions.
