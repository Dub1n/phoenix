...
When modifying CLI components (`src/cli/`):

1. **Consider command consistency** - Ensure new commands follow existing patterns
2. **Test help text** - Verify help messages are clear and accurate
3. **Test error messages** - Ensure error messages are helpful and actionable
4. **Test interactive features** - Verify prompts and menus work correctly

```bash
# CLI testing commands
phoenix-code-lite --help
phoenix-code-lite {command} --help
phoenix-code-lite {command} {invalid-args}  # Test error handling
```
