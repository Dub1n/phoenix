...
import { HandoffInput, HandoffOutput, HandoffError, HandoffErrorType } from '../interfaces/handoff-types.js';
 ...
 */
export function validateHandoffInput(data: any): ValidationResult<HandoffInput> {
  ...
  if (typeof data.context !== 'object' || data.context === null) {
    ...
    } else if (!data.context.requirements.every((req: any) => typeof req === 'string')) {
      errors.push({
        field: 'context.requirements',
    ...
    } else if (!data.context.constraints.every((constraint: any) => typeof constraint === 'string')) {
      errors.push({
        field: 'context.constraints',
    ...
    if (data.context.relevant_files !== undefined) {
      ...
      } else if (!data.context.relevant_files.every((file: any) => typeof file === 'string')) {
        errors.push({
          field: 'context.relevant_files',
  ...
  return { success: true, data: data as HandoffInput, errors: [] };
}
 ...
 * @returns Validation result with typed data or errors
 */
export function validateHandoffOutput(data: any): ValidationResult<HandoffOutput> {
  const errors: ValidationError[] = [];
  ...
  if (typeof data.results !== 'object' || data.results === null) {
    ...
    } else if (!data.results.recommendations.every((rec: any) => typeof rec === 'string')) {
      errors.push({
        field: 'results.recommendations',
    ...
    } else if (!data.results.evidence_files.every((file: any) => typeof file === 'string')) {
      errors.push({
        field: 'results.evidence_files',
  ...
  return { success: true, data: data as HandoffOutput, errors: [] };
}
 ...
 */
export function sanitizeHandoffInput(data: any): any {
  if (typeof data !== 'object' || data === null) {
    return data;
  }
  ...
  if (sanitized.context && typeof sanitized.context === 'object') {
    ...
    if (Array.isArray(sanitized.context.requirements)) {
      sanitized.context.requirements = sanitized.context.requirements.map((req: any) => 
        typeof req === 'string' ? sanitizeString(req) : req
      );
    }
    
    if (Array.isArray(sanitized.context.constraints)) {
      sanitized.context.constraints = sanitized.context.constraints.map((constraint: any) => 
        typeof constraint === 'string' ? sanitizeString(constraint) : constraint
      );
    }
  }
