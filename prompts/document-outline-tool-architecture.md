...
> NOTE: This could easily be incorporated it into one of the other tools with MCP usage enabled. I think that Haruspex would be the appropriate one for this, but it would require some thinking about. The first thing to do is to get it up and running on its own before trying to merge it with another thing.
