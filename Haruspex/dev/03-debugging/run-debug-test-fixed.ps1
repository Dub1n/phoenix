...
param(
    ...
    [Parameter(Mandatory=$false)]
    [string]$TestGuide = "dev\debugging\test-guide.md"
)
...
Write-Host "`n🏗️ Step 2: Creating Test Workspace..." -ForegroundColor Blue
...
try {
    ...
    if ($extensionList -match "haruspex") {
        ...
        Write-Host "⚠️  Haruspex extension not found. Extension needs to be packaged as VSIX first." -ForegroundColor Yellow
...
Write-Host "4. Follow the test guide: $TestGuide" -ForegroundColor Gray
Write-Host "5. Use Ctrl+Shift+P and search for 'Haruspex' commands" -ForegroundColor Gray

Write-Host "`n🎉 Setup complete! Happy debugging!" -ForegroundColor Green
