---
tags: [haruspex, dependency_framework, validation, quality_gates, vscode_compatibility]
provides: [phase_dependency_framework_1_1, enhanced_quality_gates]
requires: [./Master-Implementation-Roadmap.md, ../../docs/Haruspex_1.1.1_spec.md]
---

> Source of truth: [Haruspex 1.1](../../docs/Haruspex_1.1.1_spec.mdmd)
...
- **Phase 2** (Core Engine): Highest technical complexity, blocks all subsequent phases; must pass PCL compatibility validation
- **Phase 5** (WebView Providers): Complex UI implementation, high integration requirements; depends on Mermaid/Truth Matrix readiness
- **Phase 6** (File Monitoring): Integration point for all UI components; depends on Tree + WebViews
...
- [ ] PCLCompatibilityValidator executed; compatibility score logged via telemetry; remediation path defined
- [ ] CircuitBreaker and ErrorBoundary patterns implemented and verified
- [ ] Telemetry wired (zero PII; Output channel + status bar); key events emitted
- [ ] ≥95% unit test coverage for core components; performance targets established
...
- Compatibility validation report (telemetry snapshot)
...
- Memory thresholds & cleanup actions defined
...
- [ ] Tree view performance acceptable for large projects; UI <200ms
- [ ] Progressive enhancement mode behavior validated (feature flags honored)
...
- [ ] MermaidWebViewProvider implemented with interactive diagram viewing (lazy load/virtualization)
...
- [ ] WebView communication and event handling functional (throttle/batch messaging)
- [ ] Theme-aware styling implemented for all webviews
- [ ] Interactive features operational; UI <200ms
...
- [ ] Progressive enhancement adapts features appropriately under resource pressure
- [ ] Telemetry events emitted for monitoring; no PII
...
- [ ] VSCode version matrix validated (baseline ^1.74 + latest stable); CI hint documented
...
- [ ] Comprehensive test suite with ≥95% coverage across core components
...
- [ ] Extension validated across multiple VSCode versions and operating systems (baseline ^1.74 + latest stable)
- [ ] Telemetry verified (no PII); export command functional
...
- Test coverage ≥95% (core) with all test suites passing
