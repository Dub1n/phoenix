...
describe('Phase 8: Integration Testing & Documentation', () => {
    ...
    describe('CLI Integration Testing', () => {
        ...
        test('Debug: Test configuration functionality directly', async () => {
            ...
            const { PhoenixCodeLiteConfig } = await Promise.resolve().then(() => __importStar(require('../../src/config/settings')));
            const { ConfigFormatter } = await Promise.resolve().then(() => __importStar(require('../../src/cli/config-formatter')));
            try {
...
async function runCLICommand(args) {
    return new Promise(async (resolve) => {
        const child = (0, child_process_1.spawn)('node', ['dist/src/index.js', ...args], {
            stdio: 'pipe',
        ...
        child.on('close', async (code) => {
            if (!resolved) {
                ...
                try {
                    const { waitForCLIShutdown } = await Promise.resolve().then(() => __importStar(require('../../src/utils/test-utils')));
...
async function runCLICommandWithTimeout(args, timeoutMs) {
