...
const orchestrator_1 = require("../../src/tdd/orchestrator");
const quality_gates_1 = require("../../src/tdd/quality-gates");
const plan_test_1 = require("../../src/tdd/phases/plan-test");
const implement_fix_1 = require("../../src/tdd/phases/implement-fix");
const refactor_document_1 = require("../../src/tdd/phases/refactor-document");
const codebase_scanner_1 = require("../../src/tdd/codebase-scanner");
const client_1 = require("../../src/claude/client");
describe('Phase 3: TDD Workflow Engine', () => {
    let orchestrator;
    let mockClient;
    beforeEach(() => {
        mockClient = new client_1.ClaudeCodeClient();
        orchestrator = new orchestrator_1.TDDOrchestrator(mockClient);
    });
    describe('Quality Gate Framework', () => {
        let qualityGateManager;
        beforeEach(() => {
            qualityGateManager = new quality_gates_1.QualityGateManager();
        });
        test('Quality gates are properly initialized', () => {
            expect(qualityGateManager).toBeInstanceOf(quality_gates_1.QualityGateManager);
        });
      ...
      `;
            ...
            const report = await qualityGateManager.runQualityGates(artifact, context, 'test');
            expect(report.gateResults['code-quality'].issues.some(issue => issue.includes('long functions'))).toBe(true);
        });
    ...
    describe('Codebase Scanner', () => {
        let codebaseScanner;
        beforeEach(() => {
            codebaseScanner = new codebase_scanner_1.CodebaseScanner(mockClient);
        });
        test('Codebase scanner can be instantiated', () => {
            expect(codebaseScanner).toBeInstanceOf(codebase_scanner_1.CodebaseScanner);
        });
        ...
        test('Scan includes mandatory acknowledgment recommendation', async () => {
            ...
            expect(result.recommendations.some(rec => rec.includes('MANDATORY') || rec.includes('acknowledge'))).toBe(true);
        });
    });
    describe('TDD Phase Classes', () => {
        test('PlanTestPhase can be instantiated', () => {
            const planPhase = new plan_test_1.PlanTestPhase(mockClient);
            expect(planPhase).toBeInstanceOf(plan_test_1.PlanTestPhase);
        });
        test('ImplementFixPhase can be instantiated', () => {
            const implementPhase = new implement_fix_1.ImplementFixPhase(mockClient);
            expect(implementPhase).toBeInstanceOf(implement_fix_1.ImplementFixPhase);
        });
        test('RefactorDocumentPhase can be instantiated', () => {
            const refactorPhase = new refactor_document_1.RefactorDocumentPhase(mockClient);
            expect(refactorPhase).toBeInstanceOf(refactor_document_1.RefactorDocumentPhase);
        });
    ...
    describe('TDD Orchestrator Integration', () => {
        test('TDDOrchestrator initializes with all phases', () => {
            expect(orchestrator).toBeInstanceOf(orchestrator_1.TDDOrchestrator);
        });
        ...
        test('Workflow includes mandatory codebase scan phase', async () => {
            ...
            expect(result.metadata?.codebaseScan.recommendations.some((rec) => rec.includes('MANDATORY') || rec.includes('acknowledge'))).toBe(true);
        });
    ...
    describe('Anti-Reimplementation System', () => {
        test('Codebase scan runs before implementation phases', async () => {
            ...
            const scanLogIndex = logCalls.findIndex(log => typeof log === 'string' && log.includes('PHASE 0') && log.includes('codebase scan'));
            const planLogIndex = logCalls.findIndex(log => typeof log === 'string' && log.includes('PHASE 1') && log.includes('Planning'));
        ...
        test('Scan acknowledgment validation is enforced', async () => {
            ...
            const logCalls = consoleSpy.mock.calls.map(call => call[0]);
            const hasValidationLog = logCalls.some(log => typeof log === 'string' &&
...
//# sourceMappingURL=tdd-workflow.test.js.map