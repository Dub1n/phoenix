...
const agents_1 = require("../../src/types/agents");
const client_1 = require("../../src/claude/client");
const prompts_1 = require("../../src/claude/prompts");
describe('Phase 2: Core Architecture Integration', () => {
    describe('Structured Data Validation', () => {
        test('TaskContext schema validates correct data', () => {
            ...
            expect(() => workflow_1.TaskContextSchema.parse(validContext)).not.toThrow();
        });
        test('TaskContext schema rejects invalid data', () => {
            ...
            expect(() => workflow_1.TaskContextSchema.parse(invalidContext)).toThrow();
        });
        test('LLMResponse schema validates response structure', () => {
            ...
            expect(() => workflow_1.LLMResponseSchema.parse(validResponse)).not.toThrow();
        });
    });
    describe('Agent Specialization System', () => {
        test('All specialized agent contexts are properly defined', () => {
            expect(agents_1.SpecializedAgentContexts.PLANNING_ANALYST).toBeDefined();
            expect(agents_1.SpecializedAgentContexts.IMPLEMENTATION_ENGINEER).toBeDefined();
            expect(agents_1.SpecializedAgentContexts.QUALITY_REVIEWER).toBeDefined();
            // Verify each has required properties
            Object.values(agents_1.SpecializedAgentContexts).forEach(agent => {
    ...
    describe('Claude Code Client Integration', () => {
        let client;
        beforeEach(() => {
            client = new client_1.ClaudeCodeClient();
        });
        test('ClaudeCodeClient can be instantiated', () => {
            expect(client).toBeInstanceOf(client_1.ClaudeCodeClient);
        });
        test('validateWorkflowContext returns errors for invalid context', async () => {
            ...
            const errors = await client.validateWorkflowContext(invalidContext);
    ...
    describe('TDD Prompt System', () => {
        test('planAndTestPrompt generates structured prompt', () => {
            ...
            const prompt = prompts_1.TDDPrompts.planAndTestPrompt('Create login functionality', context);
        ...
        test('implementationPrompt includes previous context', () => {
            ...
            const prompt = prompts_1.TDDPrompts.implementationPrompt('Previous plan context', 'Test failure results', context);
...
//# sourceMappingURL=core-architecture.test.js.map