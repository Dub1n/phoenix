...
exports.calculateMenuLayout = calculateMenuLayout;
exports.renderMenuWithLayout = renderMenuWithLayout;
exports.renderSkinMenu = renderSkinMenu;
 ...
 */
function calculateMenuLayout(skinDefinition, constraints) {
 ...
 */
function renderMenuWithLayout(skinMenuDefinition, layout, context) {
    let renderedLines = 0;
    ...
    console.log(chalk_1.default.blue('💡 ') + layout.theme.descriptionStyle(generateHint(context)));
    console.log(); // Space for command prompt
}
/**
 * Core measurement functions
 */
function measureMenuContent(menuDef) {
...
function calculateOptimalWidth(measurements, constraints) {
    ...
    return Math.max(constraints.minWidth, Math.min(constraints.maxWidth, calculatedWidth));
}
function calculateConsistentHeight(measurements, constraints) {
...
function resolveTheme(skinTheme) {
    const primaryColor = skinTheme?.primaryColor || 'red';
    const accentColor = skinTheme?.accentColor || 'gray';
    return {
        titleStyle: chalk_1.default[primaryColor].bold,
        headingStyle: chalk_1.default[accentColor].bold,
        itemStyle: chalk_1.default.green,
        descriptionStyle: chalk_1.default.gray,
        separatorColor: chalk_1.default.gray
    };
}
function stripAnsi(text) {
    return text.replace(/\u001b\[[0-9;]*m/g, '');
}
function generateHint(context) {
    const hints = [];
    if (context?.level !== 'main')
        hints.push('"back" to return');
 ...
 */
function renderSkinMenu(skinMenuDefinition, skinContext, layoutConstraints // Now required - no defaults in core engine
) {
...
//# sourceMappingURL=unified-layout-engine.js.map