...
class DocumentManager {
     ...
     * ```
     */
    constructor(basePath, securityManager) {
        this.documentsPath = basePath || (0, path_1.join)(process.cwd(), '.phoenix-documents');
        this.securityManager = securityManager || new guardrails_1.SecurityGuardrailsManager();
        this.config = {
    ...
    async initializeDocumentSystem() {
        try {
            ...
            for (const dir of dirs) {
                const fullPath = (0, path_1.join)(this.documentsPath, dir);
    ...
    async scanDocuments(relativePath) {
        try {
            const fullPath = (0, path_1.join)(this.documentsPath, relativePath);
     ...
     * ```
     */
    async getTemplateDocumentConfiguration(templateName) {
        try {
            const templateData = templates_1.ConfigurationTemplates.getTemplateWithDocuments(templateName);
     ...
     * ```
     */
    async updateTemplateDocumentConfiguration(templateName, config) {
        try {
            const result = templates_1.ConfigurationTemplates.updateTemplateDocuments(templateName, config);
            return {
                success: result,
                message: result
    ...
    async searchDocuments(options) {
        ...
        if (!options.category || options.category === 'agents') {
            if (!options.agentType) {
                ...
                documents.push(...inventory.agents[options.agentType]);
            }
        }
        // Apply filters
        if (options.fileType) {
        ...
        if (options.searchTerm) {
            ...
            documents = documents.filter(doc => doc.name.toLowerCase().includes(term) ||
     ...
     * ```
     */
    async createDocumentFromTemplate(template, targetPath) {
        try {
            ...
            const targetDir = (0, path_1.join)(this.documentsPath, targetPath);
            const fullTargetPath = (0, path_1.join)(targetDir, filename);
            ...
            await fs_1.promises.writeFile(fullTargetPath, template.content, 'utf8');
            return {
                success: true,
...
`,
        ...
        for (const template of defaultTemplates) {
            ...
            const fullPath = (0, path_1.join)(this.documentsPath, targetPath, template.name + this.getExtensionForType(template.type));
    ...
    formatDisplayName(filename) {
        const name = (0, path_1.basename)(filename, (0, path_1.extname)(filename));
