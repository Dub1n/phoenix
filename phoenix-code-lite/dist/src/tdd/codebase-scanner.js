...
class CodebaseScanner {
    ...
    async scanCodebase(taskDescription, context, config) {
        ...
        try {
            ...
            const recommendations = this.generateRecommendations(relevantAssets, reuseOpportunities, conflictRisks, taskDescription);
            const result = {
                scanId,
                timestamp: new Date(),
    ...
    async discoverProjectFiles(projectPath, config) {
        const searchPrompt = `
...
`;
    ...
    async extractAssetsFromFiles(files, config) {
        const assets = [];
    ...
    async extractAssetsFromBatch(files, config) {
...
`;
    ...
    filterRelevantAssets(assets, keywords, taskDescription) {
    ...
    identifyReuseOpportunities(assets, taskDescription) {
    ...
    identifyConflictRisks(assets, taskDescription) {
        // Identify assets that might conflict with new implementation
        const taskKeywords = this.extractTaskKeywords(taskDescription);
        return assets.filter(asset => {
            // Assets with similar names might indicate potential conflicts
            const nameSimilarity = taskKeywords.some(keyword => asset.name.toLowerCase().includes(keyword) || keyword.includes(asset.name.toLowerCase()));
    ...
    generateRecommendations(relevantAssets, reuseOpportunities, conflictRisks, taskDescription) {
        const recommendations = [];
        if (reuseOpportunities.length > 0) {
            recommendations.push(`⇔ REUSE OPPORTUNITIES: Consider reusing existing assets: ${reuseOpportunities.map(a => `${a.name} (${a.filePath})`).join(', ')}`);
        }
    ...
    parseFileListFromResponse(response) {
        try {
    ...
    parseAssetsFromResponse(response) {
        try {
    ...
    isValidSourceFile(filePath, config) {
    ...
    hasSemanticSimilarity(text1, text2) {
    ...
    detectCategoryOverlap(assetType, taskDescription) {
        const task = taskDescription.toLowerCase();
        switch (assetType) {
            case 'function':
