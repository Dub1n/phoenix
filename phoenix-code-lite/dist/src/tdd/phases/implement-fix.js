...
class ImplementFixPhase {
    constructor(claudeClient) {
        this.maxAttempts = 3;
        this.claudeClient = claudeClient;
    }
    async execute(planResult, context) {
        const phase = {
            name: 'implement-fix',
            startTime: new Date(),
            success: false,
        ...
        while (attempt < this.maxAttempts) {
            ...
            try {
                ...
                const prompt = prompts_1.TDDPrompts.implementationPrompt(planResult.output || '', lastError, context);
                // Ask Claude Code to implement using Implementation Engineer persona
                const response = await this.claudeClient.query(prompt, context, agents_1.SpecializedAgentContexts.IMPLEMENTATION_ENGINEER);
    ...
    parseTestOutput(stdout, stderr, duration) {
